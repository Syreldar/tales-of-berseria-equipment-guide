...
def validate_catalogue(catalogue: Path, item_refs: list[str], allow_unbuilt: bool) -> None:
    ...
    for item in items:
        ...
        if item.get("source_kind") in {"chest_or_story", "postgame_chest_or_drop"}:
            fail(f"Catalogue acquisition must name a concrete route: {item.get('name')}")
        if "verificare" in str(item.get("acquisition", "")).lower():
            fail(f"Catalogue acquisition is still unresolved: {item.get('name')}")
